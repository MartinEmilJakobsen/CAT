...
#Tree
CATest <- CAT(Data,noise="G",workers=1,crossfit = FALSE)
g.est.Tree.graphNEL<-as(CATest$AdjMatrix, "graphNEL")
g.est.Tree.igraph<-graph_from_adjacency_matrix(CATest$AdjMatrix)

CATest.cf <- CAT(Data,noise="G",workers=1,crossfit = TRUE)
g.est.Tree.cf.graphNEL<-as(CATest.cf$AdjMatrix, "graphNEL")
g.est.Tree.cf.igraph<-graph_from_adjacency_matrix(CATest.cf$AdjMatrix)
...
saveRDS(SummaryDat,file=paste0("Data/Data_CAMvsTree_GP_Gaussian_Crossfit_",timestamp,".RDS"))
